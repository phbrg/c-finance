      ...
      {data.items.length === 0 ? (
        ...
        <>
          <div className="dashboard-toolbar">
            ...
            <label className="field compact-field"><span>Calcular até</span><input type="date" min={`${month}-01`} max={`${month}-${String(daysInMonth(month)).padStart(2, '0')}`} value={cutoffDate} onInput={(event) => onCutoffChange(event.currentTarget.value)} /></label>
          </div>
          <p className="cutoff-help">A data de corte atualiza os valores previstos e realizados até aquele dia. Os totais grandes continuam representando o mês completo.</p>
